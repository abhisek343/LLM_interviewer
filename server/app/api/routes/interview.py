...
from app.schemas.interview import (
    ...
    InterviewResultOut, SubmitAnswersRequest,
...
def get_object_id(id_str: str) -> ObjectId:
    try: return ObjectId(id_str)
        ...
        logger.error(f"Invalid ObjectId format: '{id_str}'. Error: {e}"); raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid ID format: {id_str}")

# --- Helper Dependencies for Role Checks ---
async def require_hr_or_admin(current_user: User = Depends(get_current_active_user)):
    if current_user.role not in [UserRole.hr, UserRole.admin]: raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Operation not permitted. HR or Admin privileges required.")
    return current_user

async def require_candidate(current_user: User = Depends(get_current_active_user)):
    if current_user.role != UserRole.candidate: raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Operation not permitted. Candidate role required.")
    return current_user
...
async def get_default_questions_endpoint(db: AsyncIOMotorClient = Depends(mongodb.get_db)):
    ...
    try:
        questions_cursor = db[settings.MONGODB_COLLECTION_QUESTIONS].find(); questions = await questions_cursor.to_list(length=10)
        if not questions: return []
        return [ QuestionOut(**q, question_id=str(q['_id'])) for q in questions ]
    except Exception as e: logger.error(f"Error fetching default questions: {e}", exc_info=True); raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Could not fetch default questions.")
...
async def schedule_interview(
    ...
    # (Code remains the same)
    logger.info(f"User {hr_user.username} scheduling interview for candidate {interview_data.candidate_id}"); candidate_resume_text: Optional[str] = None
    try: candidate_object_id = get_object_id(interview_data.candidate_id)
    except HTTPException as e: raise e
    candidate = await db[settings.MONGODB_COLLECTION_USERS].find_one({"_id": candidate_object_id, "role": UserRole.candidate})
    if not candidate: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Candidate user with ID {interview_data.candidate_id} not found.")
    else: candidate_resume_text = candidate.get("resume_text"); logger.info(f"Resume text found for candidate {candidate['_id']}: {bool(candidate_resume_text)}")
    logger.info(f"Generating questions for role '{interview_data.role}' and tech stack {interview_data.tech_stack}"); generated_questions_data = []
    try:
        generated_questions_data = await gemini_service.generate_questions(role=interview_data.role, tech_stack=interview_data.tech_stack, num_questions=5, resume_text=candidate_resume_text)
        if generated_questions_data: logger.info(f"Generated {len(generated_questions_data)} questions (resume context: {bool(candidate_resume_text)}).")
        else: logger.warning("Gemini service returned no questions. Falling back to defaults.")
    except Exception as e: logger.error(f"Gemini question generation failed: {e}. Falling back to defaults.", exc_info=True); generated_questions_data = []
    questions = []
    if not generated_questions_data:
        logger.info("Fetching default questions from database.")
        try:
            default_questions_cursor = db[settings.MONGODB_COLLECTION_QUESTIONS].find(); default_questions = await default_questions_cursor.to_list(length=5)
            if default_questions: questions = [ QuestionOut(**q, question_id=str(q['_id'])) for q in default_questions ]; logger.info(f"Using {len(questions)} default questions.")
            else: raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="No questions generated and no defaults available.")
        except Exception as e: raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error accessing default questions.")
    else: questions = [ QuestionOut(**q_data, question_id=str(uuid4()), created_at=datetime.utcnow()) for q_data in generated_questions_data ]
    interview_doc = interview_data.model_dump(); interview_doc["interview_id"] = str(uuid4()); interview_doc["hr_id"] = ObjectId(hr_user.id); interview_doc["status"] = "scheduled"; interview_doc["questions"] = [q.model_dump() for q in questions]; interview_doc["created_at"] = datetime.utcnow(); interview_doc["candidate_id"] = candidate_object_id
    interview_doc["overall_score"] = None; interview_doc["overall_feedback"] = None; interview_doc["completed_at"] = None; interview_doc["evaluated_by"] = None; interview_doc["evaluated_at"] = None
    try:
        result = await db[settings.MONGODB_COLLECTION_INTERVIEWS].insert_one(interview_doc); created_interview = await db[settings.MONGODB_COLLECTION_INTERVIEWS].find_one({"_id": result.inserted_id})
        if created_interview:
            created_interview["id"] = str(created_interview["_id"]); created_interview["candidate_id"] = str(created_interview["candidate_id"]); created_interview["hr_id"] = str(created_interview["hr_id"])
            for q in created_interview.get("questions", []):
                 if '_id' in q and isinstance(q.get("_id"), ObjectId): q["question_id"] = str(q["_id"])
            return InterviewOut(**created_interview)
        else: raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to retrieve created interview.")
    except Exception as e: logger.error(f"Error inserting scheduled interview into DB: {e}", exc_info=True); raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Database error occurred while scheduling interview.")
...
async def get_all_interviews(
    ...
    # (Code remains the same)
    logger.info(f"User {current_user.username} requesting all interviews. Status filter: {status_filter}"); query = {};
    if status_filter: query["status"] = status_filter
    try:
        interviews_cursor = db[settings.MONGODB_COLLECTION_INTERVIEWS].find(query); interviews = await interviews_cursor.to_list(length=None); logger.info(f"Found {len(interviews)} interviews matching filter.")
        response_list = []
        for interview in interviews:
            interview["id"] = str(interview["_id"]); interview["candidate_id"] = str(interview.get("candidate_id", "")); interview["hr_id"] = str(interview.get("hr_id", ""))
            for q in interview.get("questions", []):
                 if '_id' in q and isinstance(q.get("_id"), ObjectId): q["question_id"] = str(q["_id"])
            response_list.append(InterviewOut(**interview))
        return response_list
    except Exception as e: logger.error(f"Error fetching all interviews: {e}", exc_info=True); raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Could not retrieve interviews.")
...
async def get_all_completed_interviews(
    ...
    try:
        interviews_cursor = db[settings.MONGODB_COLLECTION_INTERVIEWS].find({"status": "completed"}); interviews = await interviews_cursor.to_list(length=None); logger.info(f"Found {len(interviews)} completed interviews.")
        response_list = []
        for interview in interviews:
            interview["id"] = str(interview["_id"]); interview["candidate_id"] = str(interview.get("candidate_id", "")); interview["hr_id"] = str(interview.get("hr_id", ""))
            for q in interview.get("questions", []):
            ...
            response_list.append(InterviewOut(**interview))
        return response_list
    except Exception as e: logger.error(f"Error fetching all completed interviews: {e}", exc_info=True); raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Could not retrieve completed interviews.")
...
async def submit_response(
    ...
    try:
        ...
        if not interview: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Interview not found, not assigned, or completed.")
        response_doc = response_data.model_dump(); response_doc["candidate_id"] = candidate_oid; response_doc["submitted_at"] = datetime.utcnow(); response_doc["score"] = None; response_doc["feedback"] = None
        ...
        if not saved_response: raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to save/retrieve response.")
        total_questions = len(interview.get("questions", [])); submitted_responses_count = await db[settings.MONGODB_COLLECTION_RESPONSES].count_documents({"interview_id": response_data.interview_id, "candidate_id": candidate_oid})
        logger.info(f"Interview {response_data.interview_id}: {submitted_responses_count}/{total_questions} responses recorded.")
        if submitted_responses_count >= total_questions > 0: await db[settings.MONGODB_COLLECTION_INTERVIEWS].update_one( {"_id": interview["_id"], "status": {"$ne": "completed"}}, {"$set": {"status": "completed", "completed_at": datetime.utcnow()}} )
        saved_response["response_id"] = str(saved_response["_id"]); saved_response["candidate_id"] = str(saved_response["candidate_id"])
        return InterviewResponseOut(**saved_response)
    except HTTPException: raise
    except Exception as e: logger.error(f"Error submitting response: {e}", exc_info=True); raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error submitting response.")
...
async def submit_all_responses(
    ...
    # (Code remains the same)
    interview_id = submission.interview_id; logger.info(f"Candidate {candidate_user.username} submitting ALL responses for interview {interview_id}.")
    try:
        candidate_oid = get_object_id(candidate_user.id); interview = await db[settings.MONGODB_COLLECTION_INTERVIEWS].find_one({ "interview_id": interview_id, "candidate_id": candidate_oid, "status": {"$ne": "completed"} })
        if not interview:
            ...
            if already_completed: raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Interview already completed.")
            else: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Interview not found or cannot be submitted.")
        delete_filter = {"interview_id": interview_id, "candidate_id": candidate_oid}; delete_result = await db[settings.MONGODB_COLLECTION_RESPONSES].delete_many(delete_filter)
        ...
        responses_to_insert = []; submitted_at = datetime.utcnow()
        for resp_data in submission.responses: response_doc = {"interview_id": interview_id, "question_id": resp_data.question_id, "answer": resp_data.answer, "candidate_id": candidate_oid, "submitted_at": submitted_at, "score": None, "feedback": None}; responses_to_insert.append(response_doc)
        if responses_to_insert: insert_result = await db[settings.MONGODB_COLLECTION_RESPONSES].insert_many(responses_to_insert); logger.info(f"Inserted {len(insert_result.inserted_ids)} new responses for interview {interview_id}.")
        else: logger.warning(f"No responses provided in the submission payload for interview {interview_id}.")
        completion_time = datetime.utcnow(); update_result = await db[settings.MONGODB_COLLECTION_INTERVIEWS].update_one({"_id": interview["_id"]}, {"$set": {"status": "completed", "completed_at": completion_time}})
        if update_result.modified_count == 1: logger.info(f"Interview {interview_id} marked as completed at {completion_time}.")
        else: logger.warning(f"Failed to mark interview {interview_id} as completed (modified count: {update_result.modified_count}).")
        return {"message": "Interview submitted successfully."}
    except HTTPException: raise
    except Exception as e: logger.error(f"Error submitting all responses: {e}", exc_info=True); raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Error submitting interview.")
...
async def get_my_interviews(
    ...
    try:
        candidate_oid = get_object_id(candidate_user.id); interviews_cursor = db[settings.MONGODB_COLLECTION_INTERVIEWS].find({"candidate_id": candidate_oid})
        interviews = await interviews_cursor.to_list(length=None); logger.info(f"Found {len(interviews)} interviews for candidate {candidate_user.username}.")
        response_list = []
        for interview in interviews:
            interview["id"] = str(interview["_id"]); interview["candidate_id"] = str(interview["candidate_id"]); interview["hr_id"] = str(interview.get("hr_id", ""))
            for q in interview.get("questions", []):
                 if '_id' in q and isinstance(q.get("_id"), ObjectId): q["question_id"] = str(q["_id"])
            response_list.append(InterviewOut(**interview))
        return response_list
    except Exception as e: logger.error(f"Error fetching interviews for candidate {candidate_user.username}: {e}", exc_info=True); raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Could not retrieve your interviews.")
...
async def get_candidate_interview_history(
    ...
    # (Code remains the same)
    logger.info(f"Fetching interview history for candidate {candidate_user.username}"); candidate_oid = get_object_id(candidate_user.id)
    try:
        ...
        if not completed_interviews: return []
        interview_history = []; interview_ids = [str(interview['interview_id']) for interview in completed_interviews]
        responses_cursor = db[settings.MONGODB_COLLECTION_RESPONSES].find({"interview_id": {"$in": interview_ids}, "candidate_id": candidate_oid})
        all_responses = await responses_cursor.to_list(length=None); responses_map: Dict[str, Dict[str, str]] = {}
        for resp in all_responses: i_id = str(resp['interview_id']); q_id = str(resp['question_id']); answer = resp.get('answer', '');
        if i_id not in responses_map: responses_map[i_id] = {}; responses_map[i_id][q_id] = answer
        for interview in completed_interviews:
            interview_id_str = str(interview['interview_id']); interview_responses = responses_map.get(interview_id_str, {}); questions_answers_list = []
            for question_data in interview.get("questions", []): q_id = str(question_data.get("question_id")); answer_text = interview_responses.get(q_id); questions_answers_list.append({"question_text": question_data.get("text", "N/A"), "answer_text": answer_text})
        ...
        logger.info(f"Returning history for {len(interview_history)} interviews for {candidate_user.username}."); return interview_history
    except Exception as e: logger.error(f"Error fetching history for {candidate_user.username}: {e}", exc_info=True); raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Could not retrieve history.")
...
async def get_single_interview_result(
    ...
    try:
        ...
        if not interview: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Completed interview {interview_id} not found.")

        candidate_id_str = str(interview.get("candidate_id"))
        # Permission check
        if current_user.role == UserRole.candidate and current_user.id != candidate_id_str: raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Permission denied.")
        ...
        logger.error(f"Error fetching result for {interview_id}: {e}", exc_info=True); raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Could not retrieve result.")
...
async def submit_interview_results(
    ...
    try:
        ...
        if not interview:
            ...
            if not_completed: raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Cannot submit results: interview not completed.")
            else: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Completed interview not found.")
        interview_oid = interview["_id"]; candidate_oid = interview["candidate_id"]
        ...
        if result_data.responses_feedback:
            ...
            for resp_feedback in result_data.responses_feedback:
                ...
                if response_update_data:
                    ...
                    if resp_update_result.matched_count == 0: logger.warning(f"No matching response found for question_id '{resp_feedback.question_id}' in interview {interview_id} to apply feedback.")
                    elif resp_update_result.modified_count > 0: updated_response_count += 1
        ...
        if all_responses:
             total_score_sum = 0; scored_responses_count = 0
             for r in all_responses:
                 score = r.get("score")
                 if score is not None:
                     try: total_score_sum += float(score); scored_responses_count += 1
                     except (ValueError, TypeError): logger.warning(f"Invalid score format '{score}' found for response {r['_id']} during overall calculation.")
             if scored_responses_count > 0: calculated_overall_score = total_score_sum / scored_responses_count
        ...
        if result_data.overall_feedback is not None: interview_update_data["overall_feedback"] = result_data.overall_feedback
        ...
        if len(interview_update_data) > 2: # Only update if score/feedback was added
             ...
             if update_result.matched_count == 0: raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Interview not found during final update.")
             if update_result.modified_count == 0: logger.warning(f"Interview {interview_id} overall results were not modified.")
             else: logger.info(f"Successfully submitted/updated overall results info for interview {interview_id} by user {hr_or_admin_user.username}.")
        else: logger.info(f"No overall score/feedback submitted for interview {interview_id} (only individual responses may have been updated).")
        ...
        if not updated_interview: raise HTTPException(status_code=500, detail="Failed to retrieve updated interview after submitting results.")
        updated_interview["id"] = str(updated_interview["_id"]); updated_interview["candidate_id"] = str(updated_interview.get("candidate_id", "")); updated_interview["hr_id"] = str(updated_interview.get("hr_id", ""))
        for q in updated_interview.get("questions", []):
             if '_id' in q and isinstance(q.get("_id"), ObjectId): q["question_id"] = str(q["_id"])
        return InterviewOut(**updated_interview)
    except HTTPException: raise
    except Exception as e: logger.error(f"Error submitting results for interview {interview_id}: {e}", exc_info=True); raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="An error occurred while submitting interview results.")
...
async def evaluate_single_response_ai(
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Invalid score format in evaluation result for response {response_id}: {ai_score}")
            raise HTTPException(
                status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
                detail="Invalid score format from AI evaluation. Score must be a number between 0 and 100."
            )
        ...
        update_data = {
            ...
            "evaluated_by": f"AI ({hr_or_admin_user.username})",
        ...
        updated_response_doc["response_id"] = str(updated_response_doc["_id"])
        updated_response_doc["candidate_id"] = str(updated_response_doc.get("candidate_id", ""))

        logger.info(f"Successfully updated response {response_id} with AI evaluation")
        return InterviewResponseOut(**updated_response_doc)
...
async def get_interview_details(
    ...
    try:
        ...
        candidate_id_str = str(interview.get("candidate_id"));
        if current_user.role == UserRole.candidate and current_user.id != candidate_id_str: raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Permission denied.")
        interview["id"] = str(interview["_id"]); interview["candidate_id"] = candidate_id_str; interview["hr_id"] = str(interview.get("hr_id", ""))
        for q in interview.get("questions", []):
             if '_id' in q and isinstance(q.get("_id"), ObjectId): q["question_id"] = str(q["_id"])
        return InterviewOut(**interview)
    except HTTPException: raise
...
async def get_interview_responses_list(
    ...
    try:
        ...
        responses = await responses_cursor.to_list(length=None); logger.info(f"Found {len(responses)} responses for interview {interview_id}")
        response_list = []
        for response in responses:
            response["response_id"] = str(response["_id"]); response["candidate_id"] = str(response.get("candidate_id", ""))
            response_list.append(InterviewResponseOut(**response))
        return response_list
    except HTTPException: raise
