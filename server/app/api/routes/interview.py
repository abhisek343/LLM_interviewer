...
from app.schemas.interview import (
    ...
    SingleResponseSubmit, # Keep the new schema
    InterviewResponseOut, # Keep this for the response model
    InterviewResultOut, SubmitAnswersRequest, AnswerItem,
    InterviewResultSubmit
    # REMOVED InterviewResponseCreate from this list
)
...
async def get_default_questions_endpoint(db: AsyncIOMotorClient = Depends(mongodb.get_db)):
    ...
    try:
        ...
        questions = await questions_cursor.to_list(length=None) # Fetch all defaults
        if not questions:
            return []
        # Use model_validate which handles aliasing (_id -> id) and field mapping
...
# --- Endpoint Signature Uses Correct Schema ---
    ...
    try:
        ...
        for interview in completed_interviews:
            ...
            for question_data in interview.get("questions", []):
                ...
                answer_text = interview_responses.get(q_id)
...
async def get_single_interview_result(
    ...
    try:
        ...
        if current_user.role == "candidate" and str(current_user.id) != candidate_id_str:
        ...
        final_score = interview.get("overall_score", calculated_score)
...
async def submit_interview_results(
    ...
    try:
        ...
        if len(interview_update_data) > 2:
...
async def evaluate_single_response_ai(
    ...
    try:
        try:
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Invalid response ID format: {response_id}")
        ...
        if missing_fields:
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail=f"Response data incomplete. Missing fields: {', '.join(missing_fields)}")
        ...
        if len(answer_text.strip()) < 10:
            ...
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Answer must be at least 10 characters long.")
        ...
        if not question_text:
            ...
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Corresponding question text not found.")

        logger.info(f"Calling Gemini service to evaluate response {response_id}...")
        try:
            evaluation_result = await gemini_service.evaluate_answer(question_text=question_text, answer_text=answer_text)
        except Exception as e:
            logger.error(f"Gemini service evaluation failed for response {response_id}: {str(e)}")
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="AI evaluation service failed. Please try again later.")

        if not isinstance(evaluation_result, dict):
            logger.error(f"Invalid evaluation result format for response {response_id}")
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Invalid evaluation result format from AI service.")
        ...
        if missing_eval_fields:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"AI evaluation incomplete. Missing fields: {', '.join(missing_eval_fields)}")
        ...
        try:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Invalid score format or range from AI evaluation.")
        ...
        try:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to update response with evaluation results.")

        if update_result.matched_count == 0:
            logger.error(f"Response not found during update: {response_id}")
            raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Response not found during update.")
        if update_result.modified_count == 0:
        ...
        if not updated_response_doc:
            ...
            raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to retrieve updated response after AI evaluation.")
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="An unexpected error occurred during AI evaluation.")
