...
async def get_current_user(token: Token, db: CurrentDB) -> UserOut:
    ...
    # --- Add Detailed DB Lookup Logging ---
    if email: # Only proceed if email was decoded
        try:
            ...
            if user_doc is None:
                ...
                # Log the ID found in the DB!
                found_user_id = str(user_doc.get('_id', 'MISSING_ID'))
                logger.debug(f"[AUTH_DEBUG] User FOUND in DB for email: {email}. DB _id: {found_user_id}")

                # Ensure the user document has an _id before validation
                if "_id" not in user_doc:
                    logger.error(f"User document for email '{email}' is missing '_id'. Potential data integrity issue.")
                ...
                try:
                    ...
                    logger.debug(f"[AUTH_DEBUG] Successfully validated user: {user.email} (Validated ID: {user.id})")
                    return user
                except ValidationError as e:
                    logger.error(f"[AUTH_DEBUG] Pydantic validation failed for user '{email}' (DB ID: {found_user_id}): {e}", exc_info=True)
                    raise internal_server_exception
